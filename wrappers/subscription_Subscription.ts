...
export function storeSubscriptionData(src: SubscriptionData) {
    return (builder: Builder) => {
        const b_0 = builder;
...
export function loadTupleSubscriptionData(source: TupleReader) {
    const _tier = source.readBigNumber();
    const _expiresAt = source.readBigNumber();
    return { $$type: 'SubscriptionData' as const, tier: _tier, expiresAt: _expiresAt };
}

export function loadGetterTupleSubscriptionData(source: TupleReader) {
    const _tier = source.readBigNumber();
    const _expiresAt = source.readBigNumber();
    return { $$type: 'SubscriptionData' as const, tier: _tier, expiresAt: _expiresAt };
}

export function storeTupleSubscriptionData(source: SubscriptionData) {
    const builder = new TupleBuilder();
...
export function dictValueParserSubscriptionData(): DictionaryValue<SubscriptionData> {
    return {
        serialize: (src, builder) => {
...
export type Subscription$Data = {
    ...
    owner: Address;
    subscriptions: Dictionary<Address, SubscriptionData>;
}

export function storeSubscription$Data(src: Subscription$Data) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeAddress(src.owner);
        b_0.storeDict(src.subscriptions, Dictionary.Keys.Address(), dictValueParserSubscriptionData());
    };
}

export function loadSubscription$Data(slice: Slice) {
    const sc_0 = slice;
    const _owner = sc_0.loadAddress();
    const _subscriptions = Dictionary.load(Dictionary.Keys.Address(), dictValueParserSubscriptionData(), sc_0);
    return { $$type: 'Subscription$Data' as const, owner: _owner, subscriptions: _subscriptions };
}

export function loadTupleSubscription$Data(source: TupleReader) {
    const _owner = source.readAddress();
    const _subscriptions = Dictionary.loadDirect(Dictionary.Keys.Address(), dictValueParserSubscriptionData(), source.readCellOpt());
    return { $$type: 'Subscription$Data' as const, owner: _owner, subscriptions: _subscriptions };
}

export function loadGetterTupleSubscription$Data(source: TupleReader) {
    const _owner = source.readAddress();
    const _subscriptions = Dictionary.loadDirect(Dictionary.Keys.Address(), dictValueParserSubscriptionData(), source.readCellOpt());
    return { $$type: 'Subscription$Data' as const, owner: _owner, subscriptions: _subscriptions };
}

export function storeTupleSubscription$Data(source: Subscription$Data) {
    const builder = new TupleBuilder();
    builder.writeAddress(source.owner);
    builder.writeCell(source.subscriptions.size > 0 ? beginCell().storeDictDirect(source.subscriptions, Dictionary.Keys.Address(), dictValueParserSubscriptionData()).endCell() : null);
    return builder.build();
}

export function dictValueParserSubscription$Data(): DictionaryValue<Subscription$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeSubscription$Data(src)).endCell());
        },
        parse: (src) => {
            return loadSubscription$Data(src.loadRef().beginParse());
        }
    }
}
...
function initSubscription_init_args(src: Subscription_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeAddress(src.owner);
    };
...
export class Subscription implements Contract {
    ...
    static async init(owner: Address) {
        return await Subscription_init(owner);
    }

    static async fromInit(owner: Address) {
        const __gen_init = await Subscription_init(owner);
        const address = contractAddress(0, __gen_init);
        return new Subscription(address, __gen_init);
    }
    ...
    async getGetSubscription(provider: ContractProvider, user: Address) {
        const builder = new TupleBuilder();
