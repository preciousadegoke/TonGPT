...
async def test_upgrade_ticket_uses_engine_units_and_user_bound_reference(engine, rail, kind):
    async def quote(path, body, **kwargs):
        ...
        return {'kind': kind, 'reference': reference, 'expectedUnits': '1333', 'expiry': '2030-01-01T00:00:00Z', 'validUntil': 1900000000, 'scheduledStart': '2030-01-01T00:00:00Z', 'scheduledExpiry': '2030-01-31T00:00:00Z'}
    ...
    assert ticket['kind'] == kind and ticket['expected_units'] == 1333
    ...
    if rail == 'ton':
        ...
        assert checkout.invoice_payload(ticket).startswith(kind + '_pro|')
...
@pytest.mark.parametrize('kind', ['upgrade', 'downgrade'])
async def test_stars_precheckout_fails_closed_before_collecting(engine, monkeypatch, mode, kind):
    pay = source('isolated_upgrade_pay', 'handlers/pay.py')
    offer = {'kind': kind, 'provider': 'telegram_stars', 'targetPlan': 'Pro', 'expectedUnits': '1333'}
    engine._get.return_value = offer
    payload = f'{kind}_pro|{REFERENCE}|777'
    amount = 1333
...
@pytest.mark.parametrize('outcome', ['held', 'queued', 'scheduled'])
@pytest.mark.parametrize('prefix', ['TGU1', 'TGD1'])
async def test_ton_upgrade_delivers_actual_amount_without_cycle_tolerance(engine, monkeypatch, timestamp, outcome, prefix):
    ...
    if outcome == 'scheduled': engine.complete_payment.return_value = {'ok': True, 'scheduled': True, 'payment_id': 'scheduled-1'}
    event = {'event_id': 'quoted-event', 'lt': 100, 'timestamp': timestamp, 'actions': [{
        'type': 'TonTransfer', 'status': 'ok', 'TonTransfer': {
            'amount': '1333', 'comment': f'{prefix}-777-pro-{REFERENCE}', 'recipient': {'address': WALLET}}}]}
    activated, confirmed = await ton._process_one_event(event)
    assert activated == 0 and confirmed is (outcome != 'queued')
...
@pytest.mark.asyncio
@pytest.mark.parametrize('rail', ['ton', 'telegram_stars'])
async def test_scheduled_receipt_queue_restart_is_not_activation(engine, tmp_path, rail):
    def load():
        q = source('isolated_scheduled_queue', 'services/activation_queue.py')
        q.QUEUE_FILE = str(tmp_path / 'scheduled.jsonl')
        q._notify_user = AsyncMock()
        return q
    q = load()
    await q.enqueue(dict(user_id=777, plan='Starter', provider=rail, external_id='scheduled-receipt',
                         duration_days=0, quote_reference=REFERENCE, paid_units=1335, paid_at='2026-09-22T12:00:00Z'))
    engine.complete_payment.return_value = {'ok': True, 'scheduled': True, 'payment_id': 'scheduled-1'}
    restarted = load()
    assert await restarted.drain_once() == 0
    assert await restarted.pending_count() == 0
    assert restarted._notify_user.call_args.args[0]['scheduled'] is True
    assert engine.complete_payment.call_args.kwargs['quote_reference'] == REFERENCE
