...
def _load_tp():
    ...
    os.environ["MONITORED_WALLET_ADDRESS"] = "EQTestWalletAddressxxxxxxxxxxxxxxxxxxxxxxxxx"
    if "services.ton_payments" in sys.modules:
        del sys.modules["services.ton_payments"]
    import importlib
    return importlib.import_module("services.ton_payments")
    ...
    events = [
        ...
        {"event_id": "ev4", "actions": [
            {"type": "TonTransfer", "status": "failed",
             "TonTransfer": {"amount": "30000000000", "comment": "TGP1-3-pro-ee33ff"}}]},  # ignored
    ]
    ...
    async def fake_fetch(limit=50):
        return [
            {"event_id": "evLow", "actions": [
                {"type": "TonTransfer", "status": "ok",
                 "TonTransfer": {"amount": "1000000000", "comment": "TGP1-9-elite-zz99zz"}}]},  # 1 TON for Elite
        ]
    tp.fetch_incoming_events = fake_fetch
